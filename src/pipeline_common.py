"""Shared v4 pipeline for single-station NCREE→struct deployments.

Consolidates the body of the previous main_v3_DXXX.py scripts. The 4
single-station mains (D001/D002/D003/D005) become 10-line wrappers that
just supply a ``StationConfig`` and the matcher's ``build_event_list``
callable. D006 still has its own dual-sensor main for now (Phase 2 will
refactor that to use this module + the inter-story deconvolution path).

Outputs go to ``BASE_DIR / f"output_v4_{station_id}"`` and the per-event
artifacts (CWT/FFT/TF/raw plots, CSV row) match the v3 schema bit-for-bit
to allow direct comparison of v3 vs v4 results during validation.
"""
...
def output_paths(cfg: StationConfig) -> dict[str, Path]:
    sid = cfg.station_id
    root = BASE_DIR / f"output_v4_{sid}"
    return {
        "root": root,
        "log": BASE_DIR / f"processing_errors_v4_{sid}.log",
        "skip_log": BASE_DIR / f"skipped_events_v4_{sid}.log",
...
def _f_min_strong_motion(signal_full, fs, segments, fmin, fmax, f_center=None):
    ...
    if f_center is not None and not np.isnan(f_center) and f_center > 0.5:
        lo = max(0.3, f_center / 1.7)
        hi = min(fs / 2 - 0.1, f_center * 1.15)
    else:
        lo, hi = fmin, fmax
    if hi - lo < 0.3:
        return float("nan"), float("nan")
...
def plot_concept_diagram(event, out_path, cfg: StationConfig):
    ...
    if not np.isnan(f_iapp) and f_iapp > 0:
        lo = max(0.3, f_iapp / 1.7); hi = min(fs / 2 - 0.1, f_iapp * 1.15)
        sos = butter(6, [lo, hi], btype="band", fs=fs, output="sos")
...
def _plot_softening_vs_pga(csv_path: Path, out_dir: Path, cfg: StationConfig):
    ...
    df["pga_n"]    = pd.to_numeric(df["pga_0m"], errors="coerce")
    ...
    axes[-1].set_xlabel("pga_0m  (gal, log scale)")
